...
- Art direction is now gated by `docs/ART_DIRECTION.md`: original cartoon-anime isometric fantasy strategy art with sampled target colors and strong black/dark-olive ink demarcation, not flat prototype terrain.
...
**Goal**: Replace the Phase 1 prototype look with the real NidoWar visual pipeline before adding movement depth. The active branch target is a cartoon-anime isometric fantasy strategy look: saturated sampled colors, cel-shaded terrain, strong black/dark-olive outlines, layered terrain materials, irregular paths, tall forests/cliffs/props, landmarks, readable heroes, and manifest-driven asset packs.
...
- New acceptance ladder:
  ...
  3. Add larger structures, stronger lighting, water, and mountain borders only after grass, then road/wall compositions read well in game.
...
- **2.R2 Target comparison diagnosis (SUPERSEDED on anime branch)** - Reframe the first gate from "bare meadow" to "target-like meadow/forest composition." The target wins because it has calm open center, dense forest borders, clustered rocks, lower-contrast grass detail, subtle dirt wear, strong shadows, and clear macro hierarchy. This is retained as historical context only; the active branch starts with grass-only cartoon/anime palette matching.
- **2.R3 Grass-only cartoon/anime pass** - Build the first accepted daylight screenshot for comparison against the target image: saturated cel-shaded lawn using sampled colors and black/dark-olive ink strokes only. No roads, walls, forest, rocks, mountains, water, landmarks, torches, night/lighting overlay, or hero.
- **2.R4 Grass base art match gate** - Freeze only when desktop and mobile screenshots prove the grass reads like the supplied cartoon/anime target. Acceptance requires user confirmation that palette, line demarcation, broad color bands, and 4X readability are close before roads.
...
- **2.R6 Road/wall composition gate** - Freeze roads/walls only when screenshots prove they improve the accepted grass composition without overwhelming the calm center. Performance target stays under 8ms p95 after warmup on browser QA.
...
- 2026-05-31: New branch `codex/anime-ink-grass-target` created for a revised art target. The first acceptance gate is now grass-only cartoon/anime isometric terrain with strong black/dark-olive outlines, not painterly pixel art and not forest composition. `docs/ASSET_PROMPTS.md` now includes sampled colors from the supplied 2026-05-31 target image: grass ink `#051005 #0c1a08 #082606`, deep grass `#0a440e #11470c #204608`, dark grass `#2d4f0c #41670a #4c6f08`, mid grass `#608306 #739507 #729708`, light grass `#84a80a #84aa0c`, dirt `#d2a353 #b08f3f #918360`, stone `#dedad2 #d2caba #c6bdab`, and water `#0172a8 #06638f #064c72`. Next implementation should blend grass color-level textures toward this exact target before adding trees, rocks, roads, walls, water, mountains, or structures.
- 2026-06-01: Anime grass/dirt terrain pass integrated. Copied the three generated grass levels into `terrain_anime_lawn_a/b/c.png` and the generated dust texture into `terrain_anime_dirt.png`, with sidecar manifests. `base-art` now blends the three grass levels, adds procedural dust islands, and draws a broken organic black ink demarcation between dust and grass. The base-art camera starts farther out (`0.56`) and the preview map is 140x140 so acceptance screenshots match the reference distance better and avoid map-edge voids. `npm run build` passed. Browser QA at `?debug&visual=base-art`: 3154 visible tiles, 0 decals, 0 props, 0 occluders, 0 torches, 56 draw calls after first chunk bake.
- 2026-06-01: Replaced procedural dust islands with the user-provided `terrain_control_layer_mask.png`. The terrain bake samples the 1672x941 mask: magenta keys to grass, white/gray keys to dirt brightness, and black keys to ink/shadow. The control mask scales to the current base-art viewport width while preserving its aspect ratio, so the target layout reads at reference distance across browser sizes. `npm run build` passed. Browser QA at `?debug&visual=base-art`: 3748 visible tiles, 0 decals, 0 props, 0 occluders, 0 torches, 64 draw calls after first chunk bake.
- 2026-06-01: Updated to the second control mask and changed `base-art` to bake the whole mask-driven terrain into one high-resolution control surface once per viewport size/zoom. Removed global pixelated canvas CSS, added linear Pixi terrain image filtering, and bilinear-sampled lawn/dirt textures during the bake to avoid source-texture aliasing. `npm run build` passed. Browser QA at `?debug&visual=base-art`: first bake around 4314.6ms, then idle around 3.5ms and pan around 5.2ms with `draw:1`, `sprites:1`, `pooled:1`, canvas CSS `auto`.
- 2026-06-01: Increased dirt texture frequency to 4x inside the mask bake so dust areas keep the original source-detail density instead of reading enlarged/pixelated. `npm run build` passed. Browser QA after reload/pan: `draw:1`, `sprites:1`, `pooled:1`, pan frame around 3.4ms.
- 2026-06-01: Swapped in the new target-specific control mask from `ChatGPT Image Jun 1, 2026, 10_49_17 AM.png`. Grass and dirt texture frequency were doubled again for a finer painted read; the control surface now covers the viewport instead of letterboxing, with an 8M pixel bake cap to prevent portrait windows from creating extreme one-time bake cost. `npm run build` passed. Browser QA at `?debug&visual=base-art`: capped first bake around 14070ms, pan around 3.3ms with `draw:1`, `sprites:1`, `pooled:1`.
- 2026-06-01: Fixed zoom rebaking in `base-art`. Control-surface sizing now uses the fixed base-art cover zoom instead of the current camera zoom, so wheel/pinch zoom scales the cached surface rather than invalidating it. Camera minimum zoom lowered below the base-art starting zoom to avoid the first zoom snapping upward. `npm run build` passed. Browser QA after first bake: zoom in around 2.8ms and zoom out around 2.5ms with `draw:1`, `sprites:1`, `pooled:1`.
- 2026-06-01: Added a lighter cartoon/anime color grade after comparing against the committed reference. Grass now lifts toward a brighter yellow-green sampled from the original lawn texture, while dirt uses a separate warm grade instead of passing through the grass grade. Dark contour pixels remain dark olive/brown rather than being globally brightened. `npm run build` passed. Browser QA after bake/pan: around 3.5ms with `draw:1`, `sprites:1`, `pooled:1`.
- 2026-06-01: Texture experiment switched `base-art` to the three user-supplied terrain sources: new bright grass (`01_51_21 PM (1)`), original grass (`May 31 09_27_36 PM (3)`), and new warmer dirt (`01_51_21 PM (3)`). The anime terrain loader now intentionally blends two lawn textures instead of requiring a third lawn source. `npm run build` passed. Browser QA after bake/pan: around 3.6ms with `draw:1`, `sprites:1`, `pooled:1`.
- 2026-06-01: Dirt texture scale adjusted from `8x` to `5x` after side-by-side review showed the new dirt source becoming too fine/flat compared with the target. This zooms the authored dirt patch/stone structure back in while keeping the brighter grass experiment. `npm run build` passed. Browser QA after bake/pan: around 3.1ms with `draw:1`, `sprites:1`, `pooled:1`.
- 2026-06-01: Cleaned control mask from `ChatGPT Image Jun 1, 2026, 04_32_27 PM.png` replaced the previous target mask for a shape-only comparison. Grass/dirt textures and grading stayed unchanged. `npm run build` passed. Browser QA after bake/pan: around 2.8ms with `draw:1`, `sprites:1`, `pooled:1`.
- 2026-06-01: Dirt scale tuning pass changed the new dirt source from `5x` to `4x` sampling, zooming the dirt artwork in further so patch structure should read larger and less dusty. `npm run build` passed. Browser QA after bake/pan: around 2.7ms with `draw:1`, `sprites:1`, `pooled:1`.
- 2026-06-01: Edge color pass replaced the near-black dirt mask base with dark olive-brown (`[43,42,16]` into `[55,48,20]` for very dark shade pixels). Goal is to keep the organic silhouette while reducing harsh black contrast against the brighter anime grass/dirt. `npm run build` passed. Browser QA after bake/pan: around 2.5ms with `draw:1`, `sprites:1`, `pooled:1`.
- 2026-06-01: Dirt readability pass added a light five-sample dirt texture filter before grading. Goal is to reduce uniform high-frequency speckle while keeping the current `4x` dirt scale, cleaned mask, grass mix, and olive-brown edge color unchanged. `npm run build` passed. Browser QA after bake/pan: around 2.6ms with `draw:1`, `sprites:1`, `pooled:1`.
- 2026-06-01: Dirt readability pass strengthened after visual comparison showed the first filter was too subtle, then rolled back after review because the stronger filter lost crispness. Current accepted state is the smaller five-sample dirt filter plus the lighter warm grade. `npm run build` passed. Browser QA after bake/pan: around 2.5ms with `draw:1`, `sprites:1`, `pooled:1`.
- 2026-06-01: Grass/herb matching pass enlarged the two lawn texture samples from `1.48/1.24` to `1.08/0.92` so the authored herb clusters and leaf patches read larger against the target. Dirt, cleaned mask, olive edge, and color grade stayed unchanged. `npm run build` passed. Browser QA after bake/pan: around 2.6ms with `draw:1`, `sprites:1`, `pooled:1`.
- 2026-06-01: Shadow-mask experiment swapped in `ChatGPT Image Jun 1, 2026, 07_29_31 PM.png` as the control layer, then rolled back after review because the prior cleaned mask read better. Current accepted mask is `ChatGPT Image Jun 1, 2026, 04_32_27 PM.png`. `npm run build` passed. Browser QA after bake/pan: around 3.2ms with `draw:1`, `sprites:1`, `pooled:1`.
- 2026-06-01: Crispness experiment changed the final Pixi filter for the baked `base-art` terrain surface from `linear` to `nearest`. Trilinear filtering would smooth scaled texture samples rather than sharpen this baked canvas path; nearest preserves baked dirt and ink pixels more aggressively. `npm run build` passed. Browser QA after bake/pan: around 2.9ms with `draw:1`, `sprites:1`, `pooled:1`.
- 2026-06-01: Herb crispness experiment changed the two lawn texture reads inside the bake from bilinear sampling to nearest sampling. Borderline/mask pixels were already nearest-sampled and the final baked surface already uses nearest filtering, so this isolates the extra crispness to herb/grass details. `npm run build` passed. Browser QA after bake/pan: around 1.7ms with `draw:1`, `sprites:1`, `pooled:1`.
- 2026-06-01: Anime herb texture experiment swapped `terrain_anime_lawn_a.png` to `ChatGPT Image Jun 1, 2026, 09_12_00 PM.png`, keeping the current underlying lawn B, dirt, cleaned mask, nearest final filtering, and nearest herb sampling. `npm run build` passed. Browser QA after bake/pan: around 3.3ms with `draw:1`, `sprites:1`, `pooled:1`.
...
- 2026-05-31: Historical visual reset note superseded on `codex/anime-ink-grass-target`. Do not lead with transition art, mountains, water, landmarks, roads/walls, forest, rocks, or structures as the first focus. The next accepted slice is grass-only cartoon/anime terrain with sampled palette and black/dark-olive ink demarcation. Runtime controls handle integration after that: alpha, tint, scale, density, contact shadows, and placement.
...
- Battle arena must follow the same `ART_DIRECTION.md` target as the world map: cartoon-anime isometric fantasy, sampled saturated palette, bold ink demarcation, embedded terrain hexes, foreground silhouettes, midground obstacles, and distant backdrop layers.
...
- `base-art` terrain now uses the authored magenta/gray control map as the single layout source. Magenta stays grass; white/gray/black samples the dirt texture with mask-driven shade/ink.
- The control-map terrain surface bakes once per viewport size/zoom into one high-resolution canvas, then Pixi draws it as one linear-filtered terrain sprite. Pan should reuse the cached surface instead of rebaking.
- Terrain canvas CSS/filtering is high-resolution art first: no global `image-rendering: pixelated`; lawn/dirt texture sampling is bilinear during the bake.
