...
export class PerformanceMonitor {
  ...
  private readonly samples = new Float32Array(HISTORY_SIZE);
  private cursor = 0;
  private count = 0;
  ...
  constructor(
    ...
    private readonly detailsNode: HTMLElement | null,
    private readonly valueNode: HTMLElement | null
  ) {
  ...
  record(frameMs: number, details: Record<string, number> | null = null): void {
  ...
  private getSamplePoint(index: number, pointCount: number, width: number, height: number): {
    sample: number;
    x: number;
    y: number;
  } {
  ...
  private paintGrid(ctx: CanvasRenderingContext2D, width: number, height: number): void {
    ctx.save();
    ctx.lineWidth = 1;
    for (const ms of [8, 16, 24, 32]) {
      const y = height - Math.min(1, ms / SAMPLE_RANGE_MS) * (height - 8) - 4;
      ctx.strokeStyle =
        ms === 32 ? 'rgba(255, 91, 64, 0.22)' :
          ms === 16 ? 'rgba(255, 170, 65, 0.22)' :
...
export function createPerformanceMonitor(): PerformanceMonitor | null {
  ...
  panel.setAttribute('aria-label', 'Frame time');
  panel.innerHTML = `
    <div class="performance-monitor__header">
      <span><strong data-performance-value>0.00</strong> ms</span>
      <span data-performance-details></span>
    </div>
    <canvas class="performance-monitor__graph" width="240" height="48"></canvas>
  `;
  document.body.append(panel);

  const monitor = new PerformanceMonitor(
    panel.querySelector<HTMLCanvasElement>('canvas.performance-monitor__graph'),
    panel.querySelector<HTMLElement>('[data-performance-details]'),
    panel.querySelector<HTMLElement>('[data-performance-value]')
  );
  monitor.resize();
...
function getSampleColor(sample: number): { glow: string; stroke: string } {
  if (sample > HEAVY_SPIKE_MS) {
    return { glow: 'rgba(255, 91, 64, 0.46)', stroke: '#ff5b40' };
  }
  if (sample > FRAME_BUDGET_MS) {
    return { glow: 'rgba(255, 170, 65, 0.42)', stroke: '#ffaa41' };
  }
  return { glow: 'rgba(73, 215, 194, 0.34)', stroke: '#74ffe1' };
}
